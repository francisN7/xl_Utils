...
class Cutter():
    ...
    def cut(self) -> None: # Faz os recortes
        ...
        self.__file_path = Path(pick_file_blocking()).absolute() # Define o diretório do arquivo
        ...
        for i in range(n): # Para cada arquivo
            ...
            self.__files.append(self.__df.iloc[inicio:fim]) # Adiciona a lista de Dataframes
        self.__save_files()
    ...
    def __save_files(self) -> None: # Salva os recortes
        ...
        output = Path(f"{self.__file_path.parent.as_posix()}/output/") # Diretório dos recortes
        ...
        for dataframe in self.__files: # Para cada dataframe
            self.__clear()
            print(":__________Recortar Planilhas__________:\n\n")
            new_path = Path.joinpath(output, f"{self.__file_path.stem}_Recorte_{n}.xlsx") # Gera o nome do arquivo
    ...
    def __lines(self) -> int: # Retorna a quantidade de linhas
        ...
        while lines <= 0: # Enquanto for menor ou igual a zero
            ...
            except Exception as e: # Se a entrada por errada!
                input(
